...
**Interpretation**: 
This means that we are 95% confident that the actual value falls within these two parameters. 
...
**Interpretation**: We're 95% the proportion of mice that died without treatment is within these two values.
...
boot_prop_lived <- mice %>% 
  filter(treat == 1) %>% 
  mutate(lived = if_else(died == 1, "no", "yes")) %>% 
...
It can be said with 90% confidence that the proportion of mice that lived while on the treatment is between 0.552 and 0.651.
